...
const User = ({ user, currentUserId }: UserProps) => {
  const messagesWithCurrentUser = user.conversations.find((conversation: TConversation) => conversation.users.find((user) => user.id === currentUserId));
  ...
  return (
    <div className="grid grid-cols-[40px_1fr_50px] grid-rows[40px] gap-3 py-3 px-4 border-b-[1px] hover:cursor-pointer hover:bg-orange-500">
      ...
      <div className="flex justify-end text-gray-500 text-xs">{latestMessage && <p>{fromNow(latestMessage.createdAt)}</p>}</div>
    </div>
  );
};
